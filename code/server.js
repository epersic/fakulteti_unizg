...
// Auth0 konfiguracija - samo dodaj req.oidc, ne štiti sve automatski
app.use(auth({
  auth0Logout: true,
  ...
  secret: process.env.AUTH0_SECRET || 'super_secret_change_this_in_production',
  authRequired: false  // NE štiti automatski
}));
...
// Ruta za osvježavanje podataka
app.post('/api/refresh-data', requiresAuth(), async (req, res) => {
  try {
    const q = {};
    const params = [];

    const facultiesSql = `SELECT id, name, short_name, established_year, website, address_street, address_city, postal_code, university FROM faculties ORDER BY id`;
    ...
    // Spremi JSON
    const jsonPath = path.join(__dirname, 'public', 'files', 'fakulteti_unizg.json');
    fs.writeFileSync(jsonPath, JSON.stringify(result, null, 2), 'utf8');

    // Spremi CSV
    const csvPath = path.join(__dirname, 'public', 'files', 'fakulteti_unizg.csv');
    const headers = ['id', 'name', 'short_name', 'established_year', 'website', 'address_street', 'address_city', 'postal_code', 'university', 'dept_name'];
    const escapeCsv = v => {
    ...
    res.json({ success: true, message: 'Data refreshed and saved' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error refreshing data' });
  }
});
